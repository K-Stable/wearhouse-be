...
public class UserPassportAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(
        ...
        UserPassportPayload payload;
        try {
            payload = decodePayload(encodedUser);
    ...
    private UserPassportPayload decodePayload(String encodedUser) {
        try {
            byte[] bytes = Base64.getUrlDecoder().decode(encodedUser);
            String json = new String(bytes, StandardCharsets.UTF_8);
            return objectMapper.readValue(json, UserPassportPayload.class);
        } catch (Exception exception) {
            throw new IllegalStateException("passport payload 파싱에 실패했습니다.", exception);
        }
    }
